<template>
  <div style="background-color: white; width: 100%; height: 100%; color: black;">

    <v-container>
      <!-- <v-row>
        <v-col md="2" cols="12">
      
            <avatar :photoURL="photoURL" />

        </v-col>
        <v-col md="10" cols="12">
          <h2>Your Name<v-btn icon>
              <v-icon color="grey">mdi-pencil-outline</v-icon>
            </v-btn>
          </h2>
          <p>Age: 20 | Location: UK</p>
          <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Obcaecati porro earum repellendus a omnis quam
            inventore quisquam saepe vel laborum, officia accusantium, corrupti id impedit enim quas eius maiores
            deleniti.<v-btn icon>
              <v-icon color="grey">mdi-pencil-outline</v-icon>
            </v-btn>
          </p>

        </v-col>
      </v-row> 
      <hr class="my-5" style="opacity: 0.5">
      <h2>Statistics</h2>
      <v-row>
          <v-col md="6" cols="12">
              <line-chart/>
          </v-col>
          <v-col md="6" cols="12">
              <line-chart/>
          </v-col>
      </v-row> -->
      <v-row class="mx-0">
        <dashboard-component style="width: 100%">
          <v-row>
              <v-col md="2" cols="12">
                  <v-img src="data:image/png;base64,"></v-img>
              </v-col>
              <v-col md="10" cols="12">
                  <h2>Your Name</h2>
                  <p>Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sed voluptates enim accusamus assumenda voluptatibus rem delectus. Tempora, enim excepturi unde id soluta vero explicabo tempore culpa quibusdam, dicta eveniet est.</p>
              </v-col>
          </v-row>
        </dashboard-component>
      </v-row>
      <v-row class="mx-0 ">
        <v-col class="pa-0" lg="6" cols="12"> 
          <dashboard-component>
            <line-chart/>
          </dashboard-component>
        </v-col>
        <v-col class="pa-0" lg="6" cols="12">
          <dashboard-component>
            <line-chart/>
          </dashboard-component>
        </v-col>
      </v-row>


    </v-container>

  </div>
</template>

<script>
  import LineChart from "@/components/LineChart.vue";
  import DashboardComponent from "@/components/DashboardComponent.vue";


  export default {
    components: {
      LineChart,
      DashboardComponent
    },
    middleware: "not-auth",
    mounted() {
      //   if (!this.$store.state.authentication.user) {
      //     this.$router.push({
      //       path: "/login"
      //     });
      //   }
    },
    computed: {
      photoURL() {
        if (this.$store.state.authentication.user) {
          return this.$store.state.authentication.user.photoURL;
        }
        return null;
      }
    }
  }

</script>

<style>

</style>
