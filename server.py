...
# stars is a 2 x password_amount array
def generateCLF(stars):
    lev_distance_matrix = []
    tsne_matrix = []
    for i in range(0, len(stars)):
        lev_distance_matrix.append([])
        tsne_matrix.append([stars[i].get("ox"), stars[i].get("oy")])
        for j in range(0, len(stars)):
            lev = textdistance.levenshtein(stars[i].get("name"), stars[j].get("name"))
    ...
    return Ridge(alpha=1.0).fit(X, tsne_matrix)
